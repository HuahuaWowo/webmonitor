...
router.get("/getpiedata", async function (ctx, next) {
  const sql = "select selector,count(*) from error where type='static' group by selector;";
...
router.get("/getbardata", async function (ctx, next) {
  ...
  const sql = "select create_time,count(*) from error where type='static' group by create_time;";
  const { data, msg, code } = await db.selectData(sql);//数据库连接获取数据
  //ctx.body赋值后就是回传给前端的数据了
  ctx.body = {
    msg,
...
router.get("/getcount", async function (ctx, next) {
  ...
  const countSql = "select count (*),count(distinct user_id) from error where type='static';";
  const { data, msg, code } = await db.selectData(countSql);
  ctx.body = {
    msg,
    code,
    data,
...
router.get("/gettabledata", async function (ctx, next) {
  ...
  const tableSql = "select id,page,selector,count(*),count(distinct user_id),max(create_time) from error where type='static' group by page;";
